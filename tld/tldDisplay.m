...
%{    
   ...
   %} 
    ...
    switch tld.plot.drawoutput
        ...
        case 1
            % jorgeb - ToDo - Confirm if/when this case occurs
            % jorgeb - ToDo - Confirm rescale is required only with bb
            % jorgeb - ToDo - Restructure to use bboxes(k).bb?
            %bb = bb_rescale_relative(bb_square(tld.bb(:,i)),[1.2 1.2]);
            %if tld.plot.confidence, bb(i) = [bb(i); tld.conf(i)]; end
            % jorgeb - Changed call to bb_draw
            ...
            bb_draw([bboxes.bb],'linewidth',linewidth,'edgecolor',color,'curvature',[1 1]);
        case 2
            % jorgeb - ToDo - Confirm if/when this case occurs
            %cp = bb_center(bb);
            cp = bb_center(tld.bb(:,i));
        ...
        case 3
            ...
            %bboxes = [bb bb2 bb3];
            % jorgeb - ToDo - Need to resolve difference between original
            % bboxes array and new bboxes struct.
            bb_draw([bboxes.bb],'linewidth',linewidth,'edgecolor',color,'curvature',[0 0]);
    end
