...
dt.conf1  = nan(1,num_dt); % Relative Similarity (for final nearest neighbor classifier)
dt.conf2  = nan(1,num_dt); % Conservative Similarity (for integration with tracker)
dt.isin   = nan(3,num_dt); % detected (isin=1) or rejected (isin=0) by nearest neighbor classifier
...
for i = 1:num_dt % for every remaining detection
    ...
    [conf1, conf2, isin] = tldNN(ex,tld); % evaluate nearest neighbor classifier
...
idx = dt.conf1 > tld.model.thr_nn; % get all indexes that made it through the nearest neighbor
