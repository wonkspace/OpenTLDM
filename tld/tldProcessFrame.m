...
[dBB dConf tld] = tldDetection(tld,I); % detect appearances by cascaded detector (variance filter -> ensemble classifier -> nearest neighbor)
...
if ~isnan(tld.bb(1,I))
   ...
   % disp(bb_center(tld.bb(:,I)));
  %  imshow((tld.img{I}.input));
%    x = size(tld.img{i}.input, 1) / 2; 
%    y = size(tld.img{i}.input, 2) / 2;
%    fprintf('center: (%d, %d)', x, y);
